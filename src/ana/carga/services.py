...
from src.ana.shared.services import LOAD_ANA_DATA_FROM_CONFIG

class LoadANADataFromConfig:
    def __init__(self, db, repository, sftp_service, control_carga_repo):
        ...
        self.base_storage_dir = STORAGE_DIR+'ana'
        self.config = {}
        self.fields_config = []
...
class ANAEventProducerFromConfig(RemoteConnectEventProducer):
...
class ANAEventConsumerFromConfig(SimpleEventConsumer):
    ...
    def execute(self, group_id=None):
        ...
        for row in cargas:
            self.ana_configs[row["queue_id"]] = row

        def map_event(event):
            event['msg_body']['config_id'] = self.ana_configs[event['queue_id']]["id"]
            return event
