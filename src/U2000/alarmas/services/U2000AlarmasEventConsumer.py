...
class U2000AlarmasEventConsumer(SimpleEventConsumer):
    def __init__(self, queue_service, service):
        super().__init__(queue_service)
        self.service = service
        self.queue_names = ['u2000.alarm_5min', 'u2000.alarm_5min.hxh']
        self.queue_handlers = {'u2000.alarm_5min': self.handler_5min, 'u2000.alarm_5min.hxh': self.handler_hxh}

    def handler_5min(self, event):
        self.__guard(event)
        
        fecha = datetime.datetime.strptime(event['msg_body']['fec_ini'], '%d/%m/%Y %H:%M')
        self.service.execute(fecha)
