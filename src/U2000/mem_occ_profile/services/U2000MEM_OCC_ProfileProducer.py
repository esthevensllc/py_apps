...
class U2000MEM_OCC_ProfileProducer(EventRemoteConnectProducer):
    def __init__(self, queue_service, remote_connect, control_carga):
        super().__init__(queue_service, remote_connect, control_carga)
        self.queue_id = 'fija.mem_occ_15min'
        self.proyect_id = 'fija.mem_occ_15min'
    ...
    def get_files_between(self, fecha_ini, fecha_fin):
        server_files = self.get_files_of('/opt/oss/server/var/neftpboot/ftproot/{}'.format(fecha_ini.strftime('%Y%m%d')), 'PM_IG80100_15')
        server_files2 = self.get_files_of('/opt/oss/server/var/neftpboot/ftproot/{}'.format(fecha_fin.strftime('%Y%m%d')), 'PM_IG80100_15')
