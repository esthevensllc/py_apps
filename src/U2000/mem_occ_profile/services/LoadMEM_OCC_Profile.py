...
class LoadMEM_OCC_Profile(AppService):
    def __init__(self, repository, remote_connect, control_carga_repo):
        ...
        self.storage_dir = STORAGE_DIR+'U2000_MEM_OCC/'
        self.proyect_carga = 'fija.mem_occ_15min'

    def execute(self, fecha):
        start_time = datetime.datetime.now()
        try:
            print("Carga 'Memory Occupancy Profile 15min'")
            str_fecha_dir = fecha.strftime('%Y%m%d')
            str_fecha_to_filter = 'PM_IG80100_15.*'+fecha.strftime('%Y%m%d%H%M')+'.*'
    ...
    def execute_hxh(self, fecha):
        print("Carga 'Memory Occupancy Profile 15min' por hora")
        str_fecha_dir = fecha.strftime('%Y%m%d')
        str_fecha_to_filter = 'PM_IG80100_15.*'+fecha.strftime('%Y%m%d%H')+'.*'
