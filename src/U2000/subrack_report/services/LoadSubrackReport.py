...
class LoadSubrackReport(AppService):
    def __init__(self, repository, remote_connect, control_carga_repo):
        ...
        self.storage_dir = STORAGE_DIR+'U2000_SUBRACK_REPORT/'
        self.proyect_carga = 'fija.subrack_rep_dia'
    
    def execute_dxd(self, fecha):
        start_time = datetime.datetime.now()
        try:
            print("Carga 'SubRack Report diario' por dia")
            str_fecha_dir = fecha.strftime('%Y%m%d')
            str_fecha_to_filter = 'Subrack_Report_'+fecha.strftime('%Y-%m-%d')+'.*'
