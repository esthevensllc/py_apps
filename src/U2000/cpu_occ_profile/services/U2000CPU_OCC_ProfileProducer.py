...
class U2000CPU_OCC_ProfileProducer(EventRemoteConnectProducer):
    ...
    def get_files_between(self, fecha_ini, fecha_fin):
        server_files = self.get_files_of('/opt/oss/server/var/neftpboot/ftproot/{}'.format(fecha_ini.strftime('%Y%m%d')), 'PM_IG80002_15')
        server_files2 = self.get_files_of('/opt/oss/server/var/neftpboot/ftproot/{}'.format(fecha_fin.strftime('%Y%m%d')), 'PM_IG80002_15')
