...
class LoadCPU_OCC_Profile(AppService):
    def __init__(self, repository, remote_connect, control_carga_repo):
        ...
        self.storage_dir = STORAGE_DIR+'U2000_CPU_OCC/'
        self.proyect_carga = 'fija.cpu_occ_15min'

    def execute_hxh(self, fecha):
        print("Carga 'CPU Occupancy Profile 15min' por hora")
        str_fecha_dir = fecha.strftime('%Y%m%d')
        str_fecha_to_filter = 'PM_IG80002_15.*'+fecha.strftime('%Y%m%d%H')+'.*'
    ...
    def execute(self, fecha):
        ...
        print("Carga 'CPU Occupancy Profile 15min'")
        try:
            str_fecha_dir = fecha.strftime('%Y%m%d')
            str_fecha_to_filter = 'PM_IG80002_15.*'+fecha.strftime('%Y%m%d%H%M')+'.*'
