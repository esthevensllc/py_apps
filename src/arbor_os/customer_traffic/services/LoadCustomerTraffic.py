...
class LoadCustomerTraffic:
    ...
    def execute(self, fecha1, fecha2):
        ...
        print("Recarga Arbor.customer_traffic {} - {}".format(str_fecha1, str_fecha2))

        # customer_traffic_response = self.__get_traffic_from_file(fecha1, fecha2)
        customer_traffic_response = self.__get_traffic_from_api(fecha1, fecha2)
        customer_traffic = customer_traffic_response['data']['attributes']['results']
        
        print("Traffic {} - {}".format(customer_traffic_response['data']['attributes']['query_start_time'], customer_traffic_response['data']['attributes']['query_end_time']))
        traffic_to_insert = []
        for traffic in customer_traffic:
    ...
    def __get_traffic_from_file(self, fecha1, fecha2):
        ...
        file = open(STORAGE_DIR+'arbor-os/traffic_queries_customer_'+str_fecha1+'__'+str_fecha2+'.json', 'r')
