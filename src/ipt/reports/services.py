...
from src.ipt.shared.services import LOAD_IPT_FROM_CONFIG
...
from src.shared.batch.pollers import PostgresPoller
import os
...
class IptProcessor(ItemProcessor):
    def __init__(self):
        self.context = dict()
...
class IptReportFromConfig:
    def __init__(self, db, repo, pg_db, control_repo):
        self.config_finder = IptConfigFinder(repo)
        self.poller = PostgresPoller(pg_db.getReference())
        self.chunk_task = ReactiveDataChunkStep(DatabaseCursorReader(), IptProcessor(), OracleWriter(db.getReference(), control_repo))
        self.event_mapper = EventMapper()
        self.wk_creator = WorkingDirectoryCreator()
        self.base_storage_dir = f"{STORAGE_DIR}ipt"
...
class IptEventProducerFromConfig(RemoteConnectEventProducer):
    def __init__(self, repository, pg_db, control_carga_repo, queue_service):
        ...
        self.finder = IptReportFinder(pg_db)
...
class IptEventConsumerFromConfig(EventConsumerFromConfig):
