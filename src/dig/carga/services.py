...
from src.dig.shared.services import LOAD_DIG_CGNAT_FROM_CONFIG
...
class DigCgnatProcessor(ListProcessor):
    def __init__(self):
        super().__init__()
        self.mapper_by_type["date"] = lambda value: self.map_str_to_datetime(value) if value is not None else None

    def process(self, items):
        items = super().process([row + [self.context['server_name_upper']] for row in items])
        return items

    def get_items_columns(self):
        return self.context['poller']['columns']+['server_name_upper']
    
    def map_str_to_datetime(self, str_date):
        fecha = dt.datetime.strptime(str_date, '%d/%m/%Y %H:%M:%S')
        return fecha


class DigCgnatReportFromConfig:
    def __init__(self, db, oracle_db, repo, control_repo, sftp_service):
        ...
        self.base_storage_dir = f"{STORAGE_DIR}dig_cgnat"
    ...
    def start(self, context):
        ...
        context['server_name_upper'] = context['config']['server_name'].upper()
        context['str_filedate'] = context['fecha_ini'].strftime('%Y%m%d')
        context['str_filemonth'] = context['fecha_ini'].strftime('%Y%m')
...
class DigCgnatEventProducerFromConfig(RemoteConnectEventProducer):
...
class DigCgnatEventConsumerFromConfig(EventConsumerFromConfig):
    def __init__(self, queue_service, app_container, notification_service, repository):
        ...
        self.handler_name = LOAD_DIG_CGNAT_FROM_CONFIG
        self.max_check_attemps = 1
        self.max_jobs_per_run = 200
