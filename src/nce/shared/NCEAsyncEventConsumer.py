...
from src.nce.shared.services import (LOAD_CSV, CLICKHOUSE_LOAD_CSV)
...
class ClickHouseNCEAsyncEventConsumer(SimpleEventConsumer):
    def __init__(self, queue_service, app_container, repository, notification_service):
        super().__init__(queue_service, app_container, notification_service)
        self.sleep_time_in_work = 0.1
        self.repository = repository
        ...
        for row in mediciones_config:
            med_gran = f"{row['codigo_medicion']}_{row['granularidad']}"
            queue_id = f'ch_nce.{med_gran}_min'.lower()
            self.queue_handlers[queue_id] = {'handler': CLICKHOUSE_LOAD_CSV, 'callback': lambda s, e: s.event_handler(map_event(e, med_gran))}
            self.medicion_by_queue[queue_id] = med_gran
