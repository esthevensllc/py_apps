...
from src.nfa.shared.services import LOAD_NFA_FROM_CONFIG
...
class NFADataFinder:
    def get_source_files(self, config, dt_fecha1, dt_fecha2):
        ...
        while dt_fecha_recorrido.strftime(config['file_date_format']) < dt_fecha2.strftime(config['file_date_format']):
            ...
            next_date = dt_fecha_recorrido + dt.timedelta(**json.loads(config['loop_time']))

            starttime = int(dt.datetime.timestamp(dt_fecha_recorrido - dt.timedelta(minutes=1)))
            endtime = int(dt.datetime.timestamp(next_date - dt.timedelta(minutes=1)))
...
class NFADataPoller(ApiDataPoller):
    ...
    def download_one(self, config, source, storage_dir):
        data = self.api.get_all(source["url"])
        if source.get('sub_url') is None:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for interface in data:
                    futures.append(executor.submit(self._download_one_sub, source, interface))
                for future in as_completed(futures):
                    data_manager.add_rows(future.result())
                source["temp_manager"] = [data_manager]

    def _download_one_sub(self, file, interface):
        sub_url = file["sub_url"].replace("{id}", f"{interface[self.id_interface_attr]}")
        return self.api.get_all(sub_url)
...
class LoadNFAFromConfig(BaseCargaFromConfig):
    def __init__(self, db, repository, sftp_service, control_carga_repo):
        ...
        self.base_storage_dir = f"{STORAGE_DIR}nfa"
        self.max_workers = 10
        self.nfa_finder = NFADataFinder()
        self.nfa_poller = NFADataPoller(self.nfa_api)
        self.nfa_processor = NFAProcessor()

    def _get_files_from_server(self, config, remote_dir, dt_fecha1, dt_fecha2):
        return self.nfa_finder.get_source_files(config, dt_fecha1, dt_fecha2)

    def _download_files(self, storage_dir, files):
        self.sources = self.nfa_poller.download(self.config, files, storage_dir)

    def _get_data_from_csv(self, fields_config, filename, skip_lines=0, date_of_file=None, env={}):
        self.config["fields"] = fields_config
        sources = self.nfa_processor.process(self.config, self.sources)
        data = []
        for src in sources:
...
class NFAEventProducerFromConfig(RemoteConnectEventProducer):
...
class NFAEventConsumerFromConfig(SimpleEventConsumer):
    ...
    def execute(self, group_id=None):
        ...
        for row in cargas:
            ...
            self.queue_handlers[queue_id] = {'handler': LOAD_NFA_FROM_CONFIG, 'callback': lambda s, e: s.event_handler(map_event(e))}
